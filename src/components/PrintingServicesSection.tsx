...
function PrintingServicesSectionContent() {
  ...
  // Загружаем категории и работы для "Полиграфические материалы"
  const { data: categories } = useCategories(locale);
  const printingCategoryId = categories?.find((c: any) => c.slug === 'poligraficheskie-materialy')?.id || '';

  const { data: worksData, isLoading } = useWorks(
    printingCategoryId
      ? { categoryId: printingCategoryId, isPublished: true }
      : { isPublished: true, limit: 0 },
    locale
  );
  const isCategoryResolving = !printingCategoryId;
  const isWorksLoading = isLoading || isCategoryResolving;
  // Услуги полиграфических материалов
  const printingServices = [
    {
      title: t('magazines'),
      image: "/Services/printing-magazines.jpg",
      description: t('magazinesDesc'),
      key: 'magazines'
    },
    {
      title: t('stickers'),
      image: "/Services/printing-stickers.jpg",
  ...
  return (
    <section className="w-full bg-[#ffffff] dark:bg-[#0b0b0b] py-24">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
              {worksData && worksData.length > 0 ? (
                worksData.map((work: any) => (
                  <WorkCard
                    key={work.id}
                    image={work.main_image_url || (work.work_images?.[0]?.image_url ?? '/Services/printing-materials.jpg')}
                    title={work.title}
      ...
      <ServiceDrawer
        ...
        serviceCategory="printingMaterials"
      />
