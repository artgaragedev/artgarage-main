...
function OutdoorServicesSectionContent() {
  ...
  // Загружаем категории и работы для "Наружная реклама"
  const { data: categories } = useCategories(locale);
  const outdoorCategoryId = categories?.find((c: any) => c.slug === 'naruzhnaya-reklama')?.id || '';

  const { data: worksData, isLoading } = useWorks(
    outdoorCategoryId
      ? { categoryId: outdoorCategoryId, isPublished: true }
      : { isPublished: true, limit: 0 },
    locale
  );
  const isCategoryResolving = !outdoorCategoryId;
  ...
  const outdoorServices = [
    {
      ...
      image: "/Services/outdoor-volumetric-letters.jpg",
      description: t('volumetricLettersDesc'),
      key: 'volumetricLetters'
    },
    {
      title: t('pseudoVolumetricLetters'),
      image: "/Services/outdoor-pseudo-volumetric-letters.jpg",
      description: t('pseudoVolumetricLettersDesc'),
      key: 'pseudoVolumetricLetters'
    },
    {
      title: t('lightbox'),
      image: "/Services/outdoor-lightbox.jpg",
      description: t('lightboxDesc'),
      key: 'lightbox'
    },
    {
      title: t('advertisingSteles'),
      image: "/Services/outdoor-advertising-steles.jpg",
      description: t('advertisingStelesDesc'),
      key: 'advertisingSteles'
    },
    {
      title: t('carBranding'),
      image: "/Services/outdoor-car-branding.jpg",
      description: t('carBrandingDesc'),
      key: 'carBranding'
    },
    {
      title: t('glassStickers'),
      image: "/Services/outdoor-glass-stickers.jpg",
      description: t('glassStickersDesc'),
      key: 'glassStickers'
    }
  ];
  ...
  return (
    <section className="w-full bg-[#ffffff] dark:bg-[#0b0b0b] py-24">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
              {worksData && worksData.length > 0 ? (
                worksData.map((work: any) => (
                  <WorkCard
                    key={work.id}
                    image={work.main_image_url || (work.work_images?.[0]?.image_url ?? '/Services/signs.jpg')}
                    title={work.title}
      ...
      <ServiceDrawer
        ...
        serviceCategory="outdoorAdvertising"
      />
...
const OutdoorServicesSection: FC = () => {
  return (
    <Suspense fallback={<div className="w-full h-screen flex items-center justify-center">Loading...</div>}>
      <OutdoorServicesSectionContent />
    </Suspense>
  );
};

export default OutdoorServicesSection;
